...
if __name__ == "__main__":
    ...
    with tf.Session(config=config) as sess:
        ...
        tr_images, tr_labels = tf.train.shuffle_batch([tr_image, tr_label], batch_size=BATCH_SIZE, capacity=100,
        ...
        val_images, val_labels = tf.train.shuffle_batch([val_image, val_label], batch_size=BATCH_SIZE, capacity=100,
                                                        min_after_dequeue=BATCH_SIZE, allow_smaller_final_batch=True)
        ...
        for epoch in range(N_EPOCHS):
            ...
            while i < NUM_IMAGES:
                ...
                if not i % 2000:
                    ...
                    print("Epoch:" + str(round(per, 2)) + "% Of " + str(epoch + 1) + "/" + str(
                        N_EPOCHS) + ", Batch loss:" + str(round(c, 4)))
                    ftime = time.time()
